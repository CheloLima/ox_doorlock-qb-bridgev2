...
function RemoveItem(playerId, item, slot)
    local QBPlayer = QBCore.Functions.GetPlayer(tonumber(playerId))

    if QBPlayer then 
        QBPlayer.Functions.RemoveItem(item, 1, slot) 
    end
end
...
function DoesPlayerHaveItem(player, items, removeItem)
    local playerId = player.source or tonumber(player)
    local QBPlayer = QBCore.Functions.GetPlayer(tonumber(playerId))
    ...
    for i = 1, #items do
        ...
        local itemData = QBPlayer.Functions.GetItemByName(itemName)

        if itemData and itemData.amount > 0 then
            if removeItem or (type(item) == "table" and item.remove) then
                QBPlayer.Functions.RemoveItem(itemName, 1, itemData.slot)
            end
